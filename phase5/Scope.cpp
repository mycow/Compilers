...
void Scope::generateLabels() {
    for(unsigned i = 0; i < _symbols.size(); i++) {
        if(_symbols[i]->type().isFunction())
            std::cout << ".globl\t" << _symbols[i]->name() << std::endl;
    }

    for(unsigned i = 0; i < _symbols.size(); i++) {
        if(_symbols[i]->type().isScalar() || _symbols[i]->type().isArray())
            std::cout << ".comm\t" << _symbols[i]->name() << ", " << _symbols[i]->type().size() << std::endl;
    }
